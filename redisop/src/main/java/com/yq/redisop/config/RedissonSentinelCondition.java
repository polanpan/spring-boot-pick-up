 ...
 */
@Configuration
public class RedissonSentinelCondition implements Condition {

    @Override
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        String redissonType = context.getEnvironment().getProperty("spring.redisson.type");
        if (RedissonTypeEnum.SENTINEL.name().equals(redissonType.trim())) {
            return true;
        } else if (!RedissonTypeEnum.SINGLE.name().equals(redissonType.trim())
